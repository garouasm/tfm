...
{ 
   ...
   // const struct attest_token_iat_simple_t  simple_claims;
    ...
    case PSA_TA_ATTESTATION_CALL:
        ...
        printf("[0][ADDRESS BUFF]   %d\r\n", &(token_buf[0]));    
        printf("[0][CONTENT BUFF]   %d\r\n", token_buf[0]);
        printf("[1][ADDRESS BUFF]   %d\r\n", &token_buf[1]);    
        printf("[1][CONTENT BUFF]   %d\r\n", token_buf[1]);

        *completed_token =
            (struct q_useful_buf_c){&(token_buf[0]), token_size};

        printf("[SWITCH]");

        printf("[0][ADDRESS PTR]    %d\r\n", completed_token->ptr);    
        printf("[0][CONTENT PTR]    %d\r\n", (uint8_t*)(completed_token->ptr));
        completed_token++;
        printf("[1][ADDRESS PTR]    %d\r\n", completed_token->ptr);    
        printf("[1][CONTENT PTR]    %d\r\n", (uint8_t*)(completed_token->ptr));
        
        print(completed_token->ptr, completed_token->len);
